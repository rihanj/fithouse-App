...
import '../../data/exception_handler/exception_handler.dart';
import '../utils/utilility.dart';
...
class NetworkUtils{
  ...
  static Future _helper(
    ...
    final response = await http.Response.fromStream(streamedResponse).timeout(Duration(seconds: timeOutDuration));
  ...
  static dynamic _parse(http.Response response) {
    ...
    try{
      ...
      throw ExceptionHandlers().getExceptionString(decoded['errors']);//(statusCode, decoded['errors'].toString());
  ...
  static Future get(Uri url, {Map<String, String>? headers,}) async {
    ...
    final response = await http.get(url, headers: headers).timeout(const Duration(seconds: timeOutDuration));
  ...
  static Future getImage(Uri url, {Map<String, String>? headers,}) async {
    _logRequest(url, 'GET', headers: headers);
